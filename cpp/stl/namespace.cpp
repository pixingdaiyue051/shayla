 ...
 * 5、类 封装
 *      将一系列属性和对属性的操作封装成一个整体 保证数据的整体性和安全性
 *      默认属性都是private只能当前类内部访问
 *      类方法默认都是内联函数 所以不宜过长 尽量只提供函数原型
 *      成员函数前加const可以限定返回值不可变即返回值是右值不可对返回值赋值 成员函数后加const可以限定函数不可变即函数内不内改变非const非static成员变量
 *      constructor 构造函数
 *          编译器会提供默认无参构造函数 如果显示创建构造函数编译器不会提供默认构造函数 需要自己补充无参构造函数
 *          如果构造函数上所有参数都有默认值 则可以省略无参构造函数
 *          单参数构造函数允许使用赋值语句默认转换成函数 可以使用explicit解除默认转换
 *      转换函数
 *          只能是类的成员函数 没有返回值没有参数
 *          用于实现将类向基本数据类型转换 可以使用explicit解除默认转换
 *      destructor  析构函数
 *          编译器会提供默认析构函数 如果显示创建析构函数编译器不会提供默认析构函数
 *          程序中不应该显示调用 程序自然结束时被动调用 如果对类成员使用指针则在delete语句之后调用
 *      继承
 *          子类可以继承父类非private成员和函数
 *          父类指针和引用可以在不进行显示类型转换下使用子类 但无法使用子类的函数
 */
#include <iostream>
#include <cstdlib>
#include <cstring>
